 ...
 * </pre>
 *
 * @author Xidan Kou & Yuanyuan Cheng
 *
 */
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (leftindex <= last) {
            int topTem = leftindex;
            if (rightindex <= last) {
                if (order.compare(array[rightindex], array[leftindex]) < 0) {
                    topTem = rightindex;
                }
            }

            if (order.compare(array[top], array[topTem]) > 0) {
                exchangeEntries(array, top, topTem);
                siftDown(array, topTem, last, order);
            }
        }
